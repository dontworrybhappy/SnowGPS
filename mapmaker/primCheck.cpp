// Uses Prim's Algorithm to if our map is connected
    ...
    */
    ...
    while (!frontier.empty()) {
        ...
        for (auto x:list[v.end]) {
            frontier.push(Edge{x.weight, x.start, x.end});
        }
    }
    if(marked.size() != vecNodes.length())
    {
        cout << "Not a connected graph! YOU FAIL! << endl;
    }
    /*if (!found) {
        return 2;
    }
